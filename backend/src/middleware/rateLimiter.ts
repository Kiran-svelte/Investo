...
function buildLimitMessage(limit: number, scope: string): { error: string; retryAfter: number } {
  return {
 ...
 */
export const companyAiRateLimiter = (req: Request, res: Response, next: NextFunction): void => {
  const user = (req as any).user;
