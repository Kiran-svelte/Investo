...
export function classifyMessageIntent(
  ...
  const isVisitStatusQuery =
    /\b(any\s+visits?|visits?\s+(booked|scheduled|for\s+me|today|this\s+week)|(do\s+i|have\s+i).*(visit|booking)|show\s+my\s+visits?|what\s+visits?|check\s+(my\s+)?visit)\b/i.test(
      lowerMessage,
    );
