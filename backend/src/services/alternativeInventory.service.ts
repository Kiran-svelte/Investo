...
function buildBudgetFilter(budgetMin?: number | null, budgetMax?: number | null) {
...
export async function searchExactProperties(criteria: PropertySearchCriteria): Promise<Property[]> {
  ...
  if (budgetFilter) Object.assign(where, budgetFilter);
...
export async function searchAlternativeTiers(criteria: PropertySearchCriteria): Promise<AlternativeTier[]> {
  ...
  if (criteria.upsellEnabled !== false && criteria.bedrooms != null && criteria.bedrooms >= 1) {
    const upsellWhere: Record<string, unknown> = {
      ...baseWhere,
      bedrooms: criteria.bedrooms + 1,
    };
    if (criteria.propertyType) upsellWhere.propertyType = criteria.propertyType;
    ...
    const upsell = await prisma.property.findMany({ where: upsellWhere as any, take: 3 });
    if (upsell.length > 0) {
      tiers.push({
        tier: 'upsell_bhk',
        properties: upsell,
  ...
  if (city) {
    const nearby = await prisma.property.findMany({
      where: {
        ...
        ...(criteria.propertyType ? { propertyType: criteria.propertyType as PropertyType } : {}),
      },
      take: 3,
  ...
  if (criteria.propertyType === 'apartment') {
    const villas = await prisma.property.findMany({
...
export function formatPropertyLine(p: Property): string {
...
export function criteriaFromLead(lead: {
  companyId: string;
  budgetMin?: unknown;
  budgetMax?: unknown;
  locationPreference?: string | null;
