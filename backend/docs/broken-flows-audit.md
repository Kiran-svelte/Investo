...
| F-13 | Buyer AI failure transparency | Staff not always notified with customer msg + AI reply | `buyerAiTransparency.util` + `staffAssist` on TurnResult + deduped alerts | `buyerAgentAssist.service.ts` | **Done** |
| F-14 | More Info wrong buttons + no media | Active visit forced Change Time; brochure only on keyword | Property detail buttons always + `resolvePropertyDetailMediaComponents` | `whatsappInteractiveOrchestrator` + `buyerSituationButtons` | **Done** |
