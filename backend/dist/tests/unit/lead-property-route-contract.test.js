...
function createRouteContractApp() {
    jest.resetModules();
    const mockPrisma = {
        lead: {
            findMany: jest.fn(),
...
describe('lead/property route contract mapping', () => {
    ...
    test('GET /api/properties returns snake_case property DTO keys', async () => {
        ...
        const response = await (0, supertest_1.default)(app).get('/api/properties');
        expect(response.status).toBe(200);
        expect(response.body.data).toHaveLength(1);
        const dto = response.body.data[0];
        expect(dto).toEqual(expect.objectContaining({
...
//# sourceMappingURL=lead-property-route-contract.test.js.map