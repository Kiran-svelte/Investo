...
describe('StorageService db-backed property import media keys', () => {
    ...
    test('verifyUploadedObject returns exists=true with metadata for db keys', async () => {
        ...
        await expect(storage_service_1.storageService.verifyUploadedObject(dbKey, { mimeType: 'application/pdf', fileSize: 3 })).resolves.toEqual({
            exists: true,
    ...
    test('verifyUploadedObject throws a 409-style error on file size mismatch for db keys', async () => {
        ...
        await expect(storage_service_1.storageService.verifyUploadedObject(dbKey, { mimeType: 'application/pdf', fileSize: 3 })).rejects.toMatchObject({
            statusCode: 409,
        });
    });
    test('getObjectBuffer returns the stored bytes for db keys', async () => {
        ...
        await expect(storage_service_1.storageService.getObjectBuffer(dbKey)).resolves.toEqual(Buffer.from('hello'));
        expect(mockPrisma.propertyImportMediaBlob.findUnique).toHaveBeenCalledWith({
            where: { mediaId },
            select: {
                bytes: true,
...
//# sourceMappingURL=storage.service.db-keys.test.js.map