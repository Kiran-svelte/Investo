...
describe('WhatsApp Service - Rich Media (CHUNK 2)', () => {
    ...
    describe('sendImage', () => {
        it('sends image successfully with caption', async () => {
            ...
            const result = await whatsappService.sendImage('+919876543210', 'https://cdn.example.com/image.jpg', 'Property exterior view', mockConfig);
        ...
        it('sends image without caption', async () => {
            ...
            const result = await whatsappService.sendImage('+919876543210', 'https://cdn.example.com/image.jpg', null, mockConfig);
            expect(result.success).toBe(true);
            const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
            expect(callBody.image.caption).toBeUndefined();
        });
        it('fails with invalid URL (not HTTPS)', async () => {
            const result = await whatsappService.sendImage('+919876543210', 'http://insecure.com/image.jpg', 'Caption', mockConfig);
            expect(result.success).toBe(false);
            expect(result.error).toBe('Image URL must be HTTPS');
            expect(mockFetch).not.toHaveBeenCalled();
        });
        it('fails with missing config', async () => {
            const result = await whatsappService.sendImage('+919876543210', 'https://cdn.example.com/image.jpg', 'Caption', { phoneNumberId: '', accessToken: '', verifyToken: '' });
            expect(result.success).toBe(false);
        ...
        it('handles API error response', async () => {
            ...
            const result = await whatsappService.sendImage('+919876543210', 'https://cdn.example.com/image.jpg', 'Caption', mockConfig);
            expect(result.success).toBe(false);
    ...
    describe('sendDocument', () => {
        it('sends PDF document successfully', async () => {
            ...
            const result = await whatsappService.sendDocument('+919876543210', 'https://cdn.example.com/brochure.pdf', 'Property_Brochure.pdf', 'Download the full brochure', mockConfig);
            expect(result.success).toBe(true);
        ...
        it('uses default filename if not provided', async () => {
            ...
            const result = await whatsappService.sendDocument('+919876543210', 'https://cdn.example.com/doc.pdf', '', null, mockConfig);
            expect(result.success).toBe(true);
            const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
            expect(callBody.document.filename).toBe('document.pdf');
        });
        it('fails with non-HTTPS URL', async () => {
            const result = await whatsappService.sendDocument('+919876543210', 'http://insecure.com/doc.pdf', 'doc.pdf', null, mockConfig);
            expect(result.success).toBe(false);
    ...
    describe('sendLocation', () => {
        it('sends location pin successfully', async () => {
            ...
            const result = await whatsappService.sendLocation('+919876543210', 12.9716, 77.5946, 'Sunshine Apartments', '123 Main Street, Whitefield, Bangalore', mockConfig);
            expect(result.success).toBe(true);
        ...
        it('fails with invalid latitude (> 90)', async () => {
            const result = await whatsappService.sendLocation('+919876543210', 91, 77.5946, 'Test', 'Address', mockConfig);
            expect(result.success).toBe(false);
            expect(result.error).toBe('Invalid coordinates');
        });
        it('fails with invalid longitude (< -180)', async () => {
            const result = await whatsappService.sendLocation('+919876543210', 12.9716, -181, 'Test', 'Address', mockConfig);
            expect(result.success).toBe(false);
            expect(result.error).toBe('Invalid coordinates');
        });
    });
    describe('sendInteractiveButtons', () => {
        it('sends buttons successfully', async () => {
            ...
            const result = await whatsappService.sendInteractiveButtons('+919876543210', 'Would you like to visit this property?', buttons, 'Property Interest', 'Reply to continue', mockConfig);
            expect(result.success).toBe(true);
            const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
        ...
        it('fails with more than 3 buttons', async () => {
            ...
            const result = await whatsappService.sendInteractiveButtons('+919876543210', 'Body text', buttons, null, null, mockConfig);
            expect(result.success).toBe(false);
            expect(result.error).toBe('Must have 1-3 buttons');
        });
        it('fails with empty buttons array', async () => {
            const result = await whatsappService.sendInteractiveButtons('+919876543210', 'Body text', [], null, null, mockConfig);
            expect(result.success).toBe(false);
            expect(result.error).toBe('Must have 1-3 buttons');
        });
        it('truncates long button titles to 20 chars', async () => {
            ...
            await whatsappService.sendInteractiveButtons('+919876543210', 'Body', buttons, null, null, mockConfig);
            const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
    ...
    describe('sendInteractiveList', () => {
        it('sends list successfully', async () => {
            ...
            const result = await whatsappService.sendInteractiveList('+919876543210', 'I found 3 properties matching your criteria:', 'View Properties', sections, 'Property Options', 'Select to know more', mockConfig);
            expect(result.success).toBe(true);
            const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
            expect(callBody.type).toBe('interactive');
        ...
        it('fails with more than 10 total rows', async () => {
            ...
            const result = await whatsappService.sendInteractiveList('+919876543210', 'Body', 'Button', sections, null, null, mockConfig);
            expect(result.success).toBe(false);
            expect(result.error).toBe('Maximum 10 rows allowed');
        });
        it('fails with empty sections', async () => {
            const result = await whatsappService.sendInteractiveList('+919876543210', 'Body', 'Button', [], null, null, mockConfig);
            expect(result.success).toBe(false);
    ...
    describe('sendPropertyImages', () => {
        it('sends up to 3 images', async () => {
            ...
            const result = await whatsappService.sendPropertyImages('+919876543210', images, 'Sunshine Apartments', mockConfig);
            expect(result.success).toBe(true);
        ...
        it('first image includes property name caption', async () => {
            ...
            await whatsappService.sendPropertyImages('+919876543210', ['https://cdn.example.com/img.jpg'], 'My Property', mockConfig);
        ...
        it('handles partial failures', async () => {
            ...
            const result = await whatsappService.sendPropertyImages('+919876543210', ['https://a.com/1.jpg', 'https://b.com/2.jpg', 'https://c.com/3.jpg'], 'Test', mockConfig);
            expect(result.success).toBe(false);
    ...
    describe('sendPropertyBrochure', () => {
        it('sends brochure with formatted filename', async () => {
            ...
            const result = await whatsappService.sendPropertyBrochure('+919876543210', 'https://cdn.example.com/brochure.pdf', 'Sunshine Apartments - Phase 2', mockConfig);
            expect(result.success).toBe(true);
            const callBody = JSON.parse(mockFetch.mock.calls[0][1].body);
...
//# sourceMappingURL=whatsapp-media.test.js.map