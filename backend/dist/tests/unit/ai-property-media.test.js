...
describe('CHUNK 5: AI Property Media Presentation', () => {
    ...
    let mockSendLocation;
    let mockSendPropertyImages;
    let mockSendPropertyBrochure;
    ...
    beforeEach(() => {
        ...
        mockSendMessage = jest.spyOn(whatsappService, 'sendMessage').mockResolvedValue(undefined);
        mockSendImage = jest.spyOn(whatsappService, 'sendImage').mockResolvedValue(undefined);
        mockSendDocument = jest.spyOn(whatsappService, 'sendDocument').mockResolvedValue(undefined);
        mockSendLocation = jest.spyOn(whatsappService, 'sendLocation').mockResolvedValue(undefined);
        mockSendPropertyImages = jest.spyOn(whatsappService, 'sendPropertyImages').mockResolvedValue(undefined);
        mockSendPropertyBrochure = jest.spyOn(whatsappService, 'sendPropertyBrochure').mockResolvedValue(undefined);
        prisma_1.default.message.create.mockResolvedValue({});
    });
    afterEach(() => {
    ...
    describe('shouldSendPropertyMedia', () => {
        it('should send media when in shortlist stage', () => {
            ...
            const result = whatsappService.shouldSendPropertyMedia(state);
        ...
        it('should send media when advancing to shortlist stage', () => {
            ...
            const action = {
                ...
                targetStage: 'shortlist',
                promptModifiers: [],
            };
            const result = whatsappService.shouldSendPropertyMedia(state, action);
            expect(result).toBe(true);
        });
    });
    describe('sendPropertyMediaSet - LAYER 1 (Visual)', () => {
        it('should send property images when available', async () => {
            ...
            await whatsappService.sendPropertyMediaSet('+919876543210', mockWhatsappConfig, mockProperty, state, 'conv-123');
            expect(mockSendPropertyImages).toHaveBeenCalledWith('+919876543210', mockProperty.images.slice(0, 3), mockProperty.name, mockWhatsappConfig);
        });
        it('should send brochure in shortlist stage', async () => {
            ...
            await whatsappService.sendPropertyMediaSet('+919876543210', mockWhatsappConfig, mockProperty, state, 'conv-123');
            expect(mockSendPropertyBrochure).toHaveBeenCalledWith('+919876543210', mockProperty.brochureUrl, mockProperty.name, mockWhatsappConfig);
        });
        it('should send location pin when coordinates available', async () => {
            ...
            await whatsappService.sendPropertyMediaSet('+919876543210', mockWhatsappConfig, mockProperty, state, 'conv-123');
            expect(mockSendLocation).toHaveBeenCalledWith('+919876543210', mockProperty.latitude, mockProperty.longitude, mockProperty.name, 'Whitefield, Bangalore, 560066', mockWhatsappConfig);
        });
        ...
        it('should send floor plans after deeper engagement', async () => {
            ...
            expect(mockSendDocument).toHaveBeenCalledWith('+919876543210', mockProperty.floorPlanUrls[0], expect.stringContaining('Floor Plan'), expect.any(String), mockWhatsappConfig);
        });
    });
    ...
    describe('sendPropertyMediaSet - LAYER 4 (Resilient)', () => {
        it('should continue if image sending fails', async () => {
            ...
            expect(prisma_1.default.message.create).toHaveBeenCalled();
        });
        it('should handle property with no images gracefully', async () => {
            ...
            await expect(whatsappService.sendPropertyMediaSet('+919876543210', mockWhatsappConfig, propertyNoImages, state, 'conv-123')).resolves.not.toThrow();
        ...
        it('should handle property with no coordinates gracefully', async () => {
            ...
            await whatsappService.sendPropertyMediaSet('+919876543210', mockWhatsappConfig, propertyNoCoords, state, 'conv-123');
    ...
    describe('sendPropertyMediaForStage - LAYER 4 (Resilient)', () => {
        it('should send fallback message if all media sending fails', async () => {
            ...
            jest.spyOn(whatsappService, 'sendPropertyMediaSet')
            ...
            await whatsappService.sendPropertyMediaForStage('+919876543210', mockWhatsappConfig, state, [mockProperty], mockLead, 'conv-123');
            // Should send fallback text message
            expect(mockSendMessage).toHaveBeenCalledWith('+919876543210', expect.stringContaining('having trouble sending the property images'), mockWhatsappConfig);
        });
        it('should rate limit between properties', async () => {
            ...
            jest.spyOn(whatsappService, 'sendPropertyMediaSet').mockResolvedValue(undefined);
            const start = Date.now();
            await whatsappService.sendPropertyMediaForStage('+919876543210', mockWhatsappConfig, state, properties, mockLead, 'conv-123');
    ...
    describe('Integration - Multiple Properties', () => {
        it('should limit to 3 properties max', async () => {
            ...
            expect(whatsappService.sendPropertyMediaSet).toHaveBeenCalledTimes(3);
        });
    });
});
//# sourceMappingURL=ai-property-media.test.js.map