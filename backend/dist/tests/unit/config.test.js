...
describe('Database configuration guard', () => {
    ...
    test('accepts a Neon connection string', () => {
        expect((0, config_1.isNeonDatabaseUrl)('postgresql://user:pass@ep-example.us-east-1.aws.neon.tech/db?sslmode=require')).toBe(true);
    });
    test('accepts a non-Neon PostgreSQL connection string', () => {
        expect((0, config_1.isNeonDatabaseUrl)('postgresql://user:pass@localhost:5432/db')).toBe(false);
    });
    test('accepts a Neon pooler connection string', () => {
        expect((0, config_1.isNeonPoolerDatabaseUrl)('postgresql://user:pass@ep-example-pooler.us-east-1.aws.neon.tech/db?sslmode=require')).toBe(true);
    });
    test('rejects a Neon direct connection string as pooler runtime URL', () => {
        expect((0, config_1.isNeonPoolerDatabaseUrl)('postgresql://user:pass@ep-example.us-east-1.aws.neon.tech/db?sslmode=require')).toBe(false);
    });
    test('throws for a non-PostgreSQL connection string', () => {
        expect(() => (0, config_1.assertValidDatabaseUrl)('mysql://user:pass@localhost:3306/db')).toThrow('DATABASE_URL must be a PostgreSQL connection string');
    });
...
describe('WhatsApp provider configuration guard', () => {
    ...
    test('defaults to meta provider when WHATSAPP_PROVIDER is unset', () => {
        ...
        expect(config.whatsapp.provider).toBe('meta');
        expect(config.greenapi.apiUrl).toBe('https://api.green-api.com');
    });
    test('allows greenapi provider in non-production', () => {
        ...
        expect(config.whatsapp.provider).toBe('greenapi');
    });
