...
function createConversationApp() {
    jest.resetModules();
    const mockPrisma = {
        conversation: {
            findMany: jest.fn(),
...
describe('conversation routes DTO shape', () => {
    ...
    test('GET /api/conversations returns snake_case conversation fields', async () => {
        ...
        const response = await (0, supertest_1.default)(app).get('/api/conversations');
        ...
        expect(response.body.data[0].last_message).toEqual(expect.objectContaining({
            content: 'Hello agent',
    ...
    test('GET /api/conversations/:id returns snake_case messages and timestamps', async () => {
        ...
        const response = await (0, supertest_1.default)(app).get('/api/conversations/conv-1');
        expect(response.status).toBe(200);
...
//# sourceMappingURL=conversation.routes.dto.test.js.map