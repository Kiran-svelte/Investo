...
const mockQueue = {
    processDueJobs: jest.fn(async (processor) => {
...
describe('PropertyImportWorkerService review transitions', () => {
    ...
    test('keeps the draft in review_ready when extraction is complete but review is pending', async () => {
        ...
        const worker = new propertyImportWorker_service_1.PropertyImportWorkerService({ pollIntervalMs: 999999 }, {
            db: mockDb,
            queue: mockQueue,
            storage: mockStorage,
...
function createWorkerWithMocks(dbOverrides = {}, storageOverrides = {}, depsOverrides = {}) {
    const db = {
        propertyImportJob: {
            findUnique: jest.fn(),
            update: jest.fn(),
    ...
    const worker = new propertyImportWorker_service_1.PropertyImportWorkerService({ pollIntervalMs: 5000 }, {
        db,
        queue,
        storage,
        now: () => now,
...
describe('Property Import Worker Service', () => {
    test('marks job succeeded idempotently when media is already extracted', async () => {
        ...
        const result = await worker.handleQueuedJob({
            payload: { jobId: 'job-1' },
        });
    ...
    test('writes extracted brochure data back into the draft for review', async () => {
        ...
        db.propertyImportDraft.findUnique.mockResolvedValue({ id: 'draft-1', status: 'extracting', draftData: {} });
        worker.deps.storage.verifyUploadedObject.mockResolvedValue({
            exists: true,
...
//# sourceMappingURL=property-import-worker.test.js.map