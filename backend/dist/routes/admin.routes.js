...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const [companiesTotal, companiesActive, usersTotal, agentsTotal, conversationsToday, messagesToday] = await Promise.all([
            ...
            prisma_1.default.company.count({ where: { status: 'active' } }),
            prisma_1.default.user.count(),
            prisma_1.default.user.count({ where: { role: 'sales_agent', status: 'active' } }),
            prisma_1.default.conversation.count({ where: { createdAt: { gte: today } } }),
            prisma_1.default.message.count({ where: { senderType: 'ai', createdAt: { gte: today } } }),
        ]);
        // Monthly revenue (sum of active companies' plan prices)
        const activeCompanies = await prisma_1.default.company.findMany({
            where: { status: 'active', planId: { not: null } },
            include: { plan: { select: { priceMonthly: true } } },
        });
        const monthlyRevenue = activeCompanies.reduce((sum, c) => sum + (Number(c.plan?.priceMonthly) || 0), 0);
        res.json({
            data: {
        ...
        logger_1.default.error('Failed to fetch admin dashboard', { error: err.message });
...
router.get('/system', async (req, res) => {
    try {
        ...
        const recentErrors = await prisma_1.default.auditLog.findMany({
        ...
        logger_1.default.error('System check failed', { error: err.message });
        res.status(500).json({
            data: {
                status: 'unhealthy',
                error: err.message,
...
router.get('/sla', async (_req, res) => {
    try {
        ...
        const [messageTotal, messageFailed, overdueInvoices, activeCompanies, stalledImports] = await Promise.all([
            prisma_1.default.message.count({ where: { createdAt: { gte: start } } }),
            prisma_1.default.message.count({ where: { createdAt: { gte: start }, status: 'failed' } }),
            prisma_1.default.invoice.count({ where: { status: 'overdue' } }),
            prisma_1.default.company.count({ where: { status: 'active' } }),
            prisma_1.default.propertyImportJob.count({
                where: {
        ...
        logger_1.default.error('Failed to compute SLA summary', { error: err.message });
...
router.get('/companies', async (req, res) => {
    try {
        ...
        // Enrich with stats
        const enriched = await Promise.all(companies.map(async ({ plan, ...company }) => {
            const [usersCount, leadsCount, conversationsCount] = await Promise.all([
                prisma_1.default.user.count({ where: { companyId: company.id, status: 'active' } }),
                prisma_1.default.lead.count({ where: { companyId: company.id } }),
                prisma_1.default.conversation.count({ where: { companyId: company.id } }),
            ]);
            return {
                ...company,
...
router.get('/usage', async (req, res) => {
    try {
        ...
        const aiUsage = await prisma_1.default.$queryRaw `
    ...
    `;
        // Get company names
        const companyIds = aiUsage.map((u) => u.company_id).filter(Boolean);
        const companies = companyIds.length > 0
            ? await prisma_1.default.company.findMany({
                where: { id: { in: companyIds } },
                select: { id: true, name: true },
            })
            : [];
        const companyMap = new Map(companies.map((c) => [c.id, c.name]));
        const usageData = aiUsage.map((u) => ({
