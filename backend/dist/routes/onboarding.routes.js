...
const PERMISSION_RESOURCE_ALIASES = {
...
function sanitizeRolePermissions(permissions) {
    ...
    for (const [resource, actions] of Object.entries(permissions)) {
        ...
        const normalizedActions = actions
            .filter((action) => typeof action === 'string' && ALLOWED_PERMISSION_ACTIONS.has(action));
...
function validateRolesPayload(rawRoles) {
    ...
    for (const role of rawRoles) {
        ...
        const rawRoleName = role.role_name;
        ...
        const rawDisplayName = role.display_name;
        const displayName = typeof rawDisplayName === 'string' && rawDisplayName.trim().length > 0
            ? rawDisplayName.trim()
            : roleName;
        const sanitizedPermissions = sanitizeRolePermissions(role.permissions);
        if (sanitizedPermissions.invalidResources.length > 0) {
            throw new Error(`Unsupported permission resources for role ${roleName}: ${sanitizedPermissions.invalidResources.join(', ')}`);
        }
        custom.set(roleName, {
            roleName,
            displayName,
...
async function assertStepPrerequisites(companyId, targetStep) {
    if (targetStep <= 1) {
        return;
    }
    const onboarding = await prisma_1.default.companyOnboarding.findUnique({
        where: { companyId },
    });
...
async function updateCompanySetup(companyId, body) {
...
router.get('/status', async (req, res) => {
    try {
        ...
        let onboarding = await prisma_1.default.companyOnboarding.findUnique({
            where: { companyId },
        });
        if (!onboarding) {
            // Create initial onboarding record
            onboarding = await prisma_1.default.companyOnboarding.create({
        ...
        res.json({
            data: {
                ...
                companyData: {
                    ...
                    primary_color: company?.settings?.primary_color || '#3B82F6',
                },
            },
        });
    }
    catch (err) {
        logger_1.default.error('Failed to get onboarding status', { error: err.message });
...
router.get('/setup', async (req, res) => {
    try {
        ...
        logger_1.default.error('Failed to fetch onboarding setup', { error: err.message });
 ...
 */
router.post('/roles', (0, rbac_1.hasRole)(...ONBOARDING_MUTATION_ROLES), async (req, res) => {
    try {
        const companyId = (0, tenant_1.getCompanyId)(req);
        ...
        for (const roleName of parsedRoles.defaults) {
            ...
            if (!existing) {
                const r = await prisma_1.default.companyRole.create({
                    data: {
                        companyId,
        ...
        logger_1.default.error('Failed to configure roles', { error: err.message });
 ...
 */
router.post('/features', (0, rbac_1.hasRole)(...ONBOARDING_MUTATION_ROLES), async (req, res) => {
    try {
        const companyId = (0, tenant_1.getCompanyId)(req);
        ...
        logger_1.default.error('Failed to configure features', { error: err.message });
        if (err.message.includes('completed first')) {
            res.status(409).json({ error: err.message });
            return;
        }
 ...
 */
router.post('/ai', (0, rbac_1.hasRole)(...ONBOARDING_MUTATION_ROLES), async (req, res) => {
    try {
        const companyId = (0, tenant_1.getCompanyId)(req);
        await assertStepPrerequisites(companyId, 4);
        const { business_name, business_description, operating_locations, budget_ranges, response_tone, persuasion_level, working_hours, faq_knowledge, greeting_template, default_language, } = req.body;
        if (!business_name) {
            res.status(400).json({ error: 'business_name is required' });
            return;
        }
        const aiSettings = await prisma_1.default.aiSetting.upsert({
            where: { companyId },
            create: {
                companyId,
        ...
        logger_1.default.error('Failed to configure AI', { error: err.message });
        if (err.message.includes('completed first')) {
            res.status(409).json({ error: err.message });
            return;
        }
 ...
 */
router.post('/invite', (0, rbac_1.hasRole)(...ONBOARDING_MUTATION_ROLES), async (req, res) => {
    try {
        const companyId = (0, tenant_1.getCompanyId)(req);
        ...
        for (const m of members) {
            if (!m.name || !m.email || !m.role)
                continue;
            const normalizedEmail = (0, auth_service_1.normalizeAuthEmail)(String(m.email));
        ...
        logger_1.default.error('Failed to invite team', { error: err.message });
        if (err.message.includes('completed first')) {
            res.status(409).json({ error: err.message });
            return;
        }
 ...
 */
router.post('/complete', (0, rbac_1.hasRole)(...ONBOARDING_MUTATION_ROLES), async (req, res) => {
    try {
        const companyId = (0, tenant_1.getCompanyId)(req);
        await assertStepPrerequisites(companyId, 6);
        const onboarding = await prisma_1.default.companyOnboarding.upsert({
            where: { companyId },
            create: {
                companyId,
                stepCompleted: 6,
        ...
        logger_1.default.error('Failed to complete onboarding', { error: err.message });
        if (err.message.includes('completed first')) {
            res.status(409).json({ error: err.message });
            return;
        }
