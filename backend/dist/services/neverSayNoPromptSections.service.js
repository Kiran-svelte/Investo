...
exports.buildIntentScenarioSections = buildIntentScenarioSections;
exports.buildCompetitorReferralSection = buildCompetitorReferralSection;
exports.buildWaitlistSection = buildWaitlistSection;
exports.formatExternalPartnersForPrompt = formatExternalPartnersForPrompt;
const never_say_no_constants_1 = require("../constants/never-say-no.constants");
function buildIntentScenarioSections(input) {
    const lines = [];
    ...
    if (intents.wantsPanIndia) {
        ...
        lines.push(never_say_no_constants_1.PAN_INDIA_RESPONSE.replace('{city}', city).replace('Pune, Hyderabad, Chennai, Mumbai', settings.pan_india_cities.join(', ')));
        lines.push('');
    }
    if (intents.wantsInternational && settings.international_partners_enabled) {
        lines.push('### ✈️ INTERNATIONAL REQUEST');
        lines.push(never_say_no_constants_1.INTERNATIONAL_RESPONSE);
        lines.push('');
    }
    if (intents.urgentPossession) {
        lines.push('### ⏱️ POSSESSION URGENCY');
        for (const template of never_say_no_constants_1.POSSESSION_GAP_STRATEGIES) {
            lines.push(`  - ${template
    ...
    if (intents.wantsToBuy && settings.business_type === 'rental' && settings.offer_rent_to_own) {
        ...
        lines.push(`  - Rent for ${settings.rent_to_own_months} months; if they buy within that window, deduct ${settings.rent_credit_percent}% of total rent from purchase price.`);
    ...
    if (!hasOwnInventory && !hasPartnerInventory && activePartners.length > 0) {
        ...
        lines.push("  → Say: 'I've checked our partner network — they may have exactly what you need. I earn a referral fee but you pay the same market price.'");
        lines.push('');
    }
    if (!hasOwnInventory && settings.portal_search_enabled) {
        lines.push('### 🔍 PUBLIC LISTING NETWORK (MagicBricks / 99acres / Housing.com)');
        lines.push('  - Our team monitors public listings daily for matches to client criteria.');
        lines.push("  - Say: 'I'll search top portals now and connect you to the listing agent — you stay in this chat, I negotiate referral and support.'");
        lines.push('');
    }
    if (intents.notInterested) {
        lines.push('### 💬 NOT INTERESTED');
        lines.push("  → Ask: 'When is a good time to follow up?' Offer monthly market updates for their area.");
        lines.push('');
    }
    ...
    if (intents.foundElsewhere) {
        ...
        lines.push("  → Offer FREE legal verification before they sign — no obligation. '" +
            never_say_no_constants_1.VALUE_ADD_SERVICES[0] +
            "'");
        lines.push('');
    }
    ...
    lines.push(`  - 3d: ${never_say_no_constants_1.RE_ENGAGEMENT_TEMPLATES[3].replace('{name}', input.customerName || 'there').replace('{area}', area)}`);
    lines.push(`  - 7d: ${never_say_no_constants_1.RE_ENGAGEMENT_TEMPLATES[7].replace('{name}', input.customerName || 'there').replace('{area}', area)}`);
    lines.push(`  - 30d: ${never_say_no_constants_1.RE_ENGAGEMENT_TEMPLATES[30].replace('{name}', input.customerName || 'there').replace('{area}', area)}`);
    lines.push('');
    return lines;
}
function buildCompetitorReferralSection(settings) {
    const template = never_say_no_constants_1.SCENARIO_COMPETITOR_REFERRAL.messageTemplate
        .replace('{competitorName}', settings.competitor_name)
        .replace('{referralDiscount}', String(settings.referral_discount_percent));
    return (`### 🎯 LAST RESORT — COMPETITOR REFERRAL (Tier 9)\n` +
...
function buildWaitlistSection(settings, language) {
    const copy = settings.waitlist_copy[language] || settings.waitlist_copy.en;
    return (`### 📋 WAITLIST (Always available)\n` +
        `${copy}\n` +
        `Typical wait: 2–4 weeks. Capture BHK, area, budget in conversation commitments.`);
}
function formatExternalPartnersForPrompt(partners) {
    return partners
        .filter((p) => p.active)
