...
class AIService {
    ...
    async generateResponse(request) {
        ...
        if ((0, buyerVisitQuery_service_1.isBuyerVisitStatusQuery)(request.customerMessage)
            && request.companyId
            && request.lead?.id) {
            const buyerLang = (0, buyerI18n_util_1.resolveBuyerLanguage)({
        ...
        if (fastPath) {
            ...
            if (!containsBannedBuyerPhrase(fastPath.text, { hasPriorOutbound, stage: newState.stage })) {
                fastPath.detectedLanguage = (0, buyerI18n_util_1.resolveBuyerLanguage)({
                    message: request.customerMessage,
                    leadLanguage: request.lead?.language,
                });
                return {
        ...
        for (const provider of providers) {
            ...
            try {
                ...
                response.detectedLanguage = (0, buyerI18n_util_1.resolveBuyerLanguage)({
                    message: request.customerMessage,
                    leadLanguage: request.lead?.language,
                });
        ...
        mockResp.detectedLanguage = (0, buyerI18n_util_1.resolveBuyerLanguage)({
            message: request.customerMessage,
            leadLanguage: request.lead?.language,
        });
        return mockResp;
