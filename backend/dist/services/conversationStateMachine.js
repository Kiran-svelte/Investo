...
class PolicyBrain {
     ...
     */
    decideNextAction(state, messageIntent, objectionType, customerMessage) {
        ...
        const authorityTopic = customerMessage ? (0, realEstateAssistantPrompt_constants_1.detectAuthorityLimitTopic)(customerMessage) : null;
        if (authorityTopic) {
    ...
    shouldEscalate(state, intent, objectionType) {
        // Price / negotiation → human (hybrid 10% path)
        if ((objectionType === 'price_too_high' || state.lastObjectionType === 'price_too_high')
    ...
    getEscalationReason(state, intent, objectionType) {
        if ((objectionType === 'price_too_high' || state.lastObjectionType === 'price_too_high')
            && state.consecutiveObjections >= 2) {
    ...
    canAdvanceStage(state) {
        const stageConfig = STAGE_CONFIG[state.stage];
        // Check if required commitments are met
        const requiredMet = stageConfig.requiredCommitments.every((c) => state.commitments[c]);
        return requiredMet;
    }
...
exports.PolicyBrain = PolicyBrain;
// ─── State Manager ─────────────────────────────────────────────────────
class ConversationStateManager {
    constructor() {
        this.policyBrain = new PolicyBrain();
    }
    /**
     ...
     */
    processMessage(currentState, message, extractedInfo) {
        // Classify the message intent
        const { intent, objectionType, confidence } = classifyMessageIntent(message, currentState.stage, { consecutiveObjections: currentState.consecutiveObjections });
        logger_1.default.info('Message classified', {
            intent,
            objectionType,
            confidence,
    ...
    updateState(state, intent, objectionType, extractedInfo) {
     ...
     */
    calculateUrgencyScore(state, timeline) {
        let score = 5;
        // Timeline mentions
        if (timeline) {
        ...
        // High engagement
        if (state.commitments.propertyInterestShown)
            score += 1;
        if (state.commitments.visitSlotDiscussed)
            score += 2;
...
exports.policyBrain = new PolicyBrain();
