...
exports.buildAddressFromProperty = buildAddressFromProperty;
...
function getCachedResult(address) {
    if (!config_1.default.geocoding.cacheEnabled)
        return null;
    ...
    const ttlMs = config_1.default.geocoding.cacheTtlSeconds * 1000;
    if (ageMs > ttlMs) {
...
async function geocodeWithGoogle(address) {
    const apiKey = config_1.default.geocoding.googleApiKey;
    if (!apiKey) {
    ...
    try {
        ...
        if (!response.ok) {
            logger_1.default.error('Google Geocoding API error', { status: response.status });
            return null;
        }
        const data = await response.json();
        if (data.status !== 'OK' || !data.results.length) {
            logger_1.default.info('Google Geocoding no results', { status: data.status, address });
            return null;
        }
        ...
        logger_1.default.error('Google Geocoding failed', { error: err.message });
        return null;
    }
}
/**
...
async function geocodeWithNominatim(address) {
    ...
    try {
        ...
        const response = await fetch(url.toString(), {
            headers: {
                'User-Agent': config_1.default.geocoding.nominatimUserAgent,
            },
        });
        if (!response.ok) {
            logger_1.default.error('Nominatim API error', { status: response.status });
            return null;
        }
        const data = await response.json();
        if (!data.length) {
            logger_1.default.info('Nominatim no results', { address });
            return null;
        }
        const result = data[0];
        ...
        logger_1.default.error('Nominatim Geocoding failed', { error: err.message });
        return null;
    }
}
/**
...
async function geocodeAddress(address) {
    ...
    if (cached) {
        logger_1.default.debug('Geocoding cache hit', { address: address.substring(0, 50) });
        return cached;
    }
    let result = null;
    // Try configured provider first
    if (config_1.default.geocoding.provider === 'google') {
        ...
        if (!result && config_1.default.geocoding.googleApiKey) {
            result = await geocodeWithGoogle(address);
        }
    }
    ...
    if (result) {
        ...
        logger_1.default.info('Geocoding success', {
        ...
        logger_1.default.warn('Geocoding failed for address', { address: address.substring(0, 100) });
    }
    return result;
}
/**
...
function buildAddressFromProperty(property) {
    ...
    if (property.locationArea?.trim())
        parts.push(property.locationArea.trim());
    if (property.locationCity?.trim())
        parts.push(property.locationCity.trim());
    if (property.locationPincode?.trim())
        parts.push(property.locationPincode.trim());
...
function getGeocodeStats() {
    return {
        ...
        enabled: config_1.default.geocoding.cacheEnabled,
    };
}
