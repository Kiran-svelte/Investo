...
async function main() {
  ...
  const target = targets.find((entry) => entry.type === 'page' && entry.url.includes('127.0.0.1:5181')) || targets.find((entry) => entry.type === 'page');
  ...
  await send('Page.navigate', { url: 'http://127.0.0.1:5181/login' });
