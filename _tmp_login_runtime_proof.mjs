...
async function main() {
  const loginUrl = 'https://investo-frontend-in3m.onrender.com/login';

  const targets = await waitForJson('http://127.0.0.1:9222/json/list');
  const pageTarget =
    targets.find((entry) => entry.type === 'page' && entry.url.includes('investo-frontend-in3m.onrender.com')) ||
  ...
  if (!ready) throw new Error('Login form not ready');
  ...
  const loginSucceeded = finalState.pathname !== '/login' && finalState.pathname !== '/login/';

  console.log(
    JSON.stringify(
      {
        attemptedUrl: loginUrl,
        credentials: { email: 'admin@investo.in', password: 'admin@123' },
        loginSucceeded,
