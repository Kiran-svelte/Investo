...
$hdrLines = & curl.exe -sSi -D - -o NUL -H "x-vercel-protection-bypass: $token" -H "Accept: text/html" $u
...
$cookieHeaders = $lines | Where-Object { $_ -match "^(?i)Set-Cookie:" }
$cookieNames = $cookieHeaders | ForEach-Object { ($_ -replace "^(?i)Set-Cookie:\s*","").Split(";")[0].Split("=")[0] } | Sort-Object -Unique
[pscustomobject]@{ url=$u; finalStatus=$finalStatus; cookieNames=$cookieNames; tokenLength=$token.Length; allHeaders=$lines } | ConvertTo-Json -Depth 4
