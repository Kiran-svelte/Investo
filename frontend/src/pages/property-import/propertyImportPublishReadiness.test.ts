...
describe('getPublishReadiness', () => {
  ...
  it('warns when city and area are both missing', () => {
    const result = getPublishReadiness({
      formValues: {
        ...PROPERTY_IMPORT_DEFAULT_FORM_VALUES,
        name: 'Towers',
    ...
    expect(result.warnings).toContain(
      'Add city or area when you know it. This helps location matching on WhatsApp.',
    );
  });

  it('does not warn when city or area is provided', () => {
    const result = getPublishReadiness({
      formValues: {
        ...PROPERTY_IMPORT_DEFAULT_FORM_VALUES,
        name: 'Towers',
